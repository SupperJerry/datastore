...
@Service
public class LFSMonitor implements IMonitor {

    private final Logger logger = LogManager.getLogger(LFSMonitor.class);
    ...
    public LFSMonitor() throws IOException {
            ...
            public void run() {
                while(runMonitor) {
                    ...
                    for (WatchEvent<?> event: key.pollEvents()) {
                        ...
                        FileUpdateMessage directoryUpdateMessage;
                        ...
                        if (kind == OVERFLOW) {
                            ...
                            directoryUpdateMessage = new FileUpdateMessage(fileName, parentPath, Constants.FILE_CREATED);
                            dispatchQueue.addMsgToQueue(directoryUpdateMessage);
                        } else if (kind == ENTRY_DELETE) {
                            // file delete event
                            logger.info("File deleted. File name: "+fileName.toString());
                            directoryUpdateMessage = new FileUpdateMessage(fileName, parentPath, Constants.FILE_DELETED);
                            dispatchQueue.addMsgToQueue(directoryUpdateMessage);
                        } else if (kind == ENTRY_MODIFY) {
                            // file modify event
                            logger.info("File modified. File name: "+fileName.toString());
                            directoryUpdateMessage = new FileUpdateMessage(fileName, parentPath, Constants.FILE_MODIFIED);
                            dispatchQueue.addMsgToQueue(directoryUpdateMessage);
                        };
                    }
